...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " qstees-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  qstees-tx [options] <hex-tx> [commands]  Update hex-encoded qstees transaction\n" +
            "or:     qstees-tx [options] -create [commands]   Create hex-encoded qstees transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded qstees transaction
